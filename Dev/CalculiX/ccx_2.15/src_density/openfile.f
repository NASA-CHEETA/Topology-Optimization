...
c      logical exi
      character*3 output
      character*132 jobname,fndat,fnfrd,fnsta,fncvg,fncel,fncsv
      integer i
!
...
c      endif
!
      fncsv=jobname(1:i)//'.csv'
      open(5,file=fncsv(1:i+4),status='replace',err=51)
!     close(5,status='delete',err=52)
!     open(5,file=fncsv(1:i+4),status='new',err=51)
!
...
c      call exit(201)
 51   write(*,*) '*ERROR in openfile: could not open file ',fncsv(1:i+4)
      call exit(201)
