...
//    fprintf(f1,"%5s%1s\n",p1,c);
    ...
    fprintf(f1,"%5sUVERSION           MDOCC                             \n",p1);
    fprintf(f1,"%5sUCOMPILETIME       Thu Jun 24 02:49:31 CDT 2021                    \n",p1);
    fprintf(f1,"%5sUDIR                                                               \n",p1);
    fprintf(f1,"%5sUDBN                                                               \n",p1);
...
//	  /* MASS: store nothing */
  ...
  // Write nodal displacement to a separate file

FILE *fptr2;
fptr2 = fopen("Disp.csv", "w");
if (fptr2 == NULL)
{
	printf("ERROR! Unable to open displacement write-file.\n");
	exit(1);
}
	fprintf(fptr2, " Ux  Uy  Uz  \n");
...
//	  frdheader(&icounter,&oner,time,&pi,noddiam,cs,&null,mode,
//		    &noutloc,description,kode,nmethod,fptr2,output,istep,iinc);
    ...
    {
	      frdvector(v,&iset,ntrans,filab,&nkcoords,inum,m1,inotr,
		    trab,co,istartset,iendset,ialset,mi,ngraph,fptr2,output,m3);
	  }
  ...
  //		    nfieldtensor,&iselect,m2,fptr,output,m3);
...
// Write noal stress to a separate file

FILE *fptr;
fptr = fopen("Stress.csv", "w");
if (fptr == NULL)
{
	printf("ERROR! Unable to open write-file.\n");
	exit(1);
}
	fprintf(fptr, " Sxx  Syy  Szz  Sxy  Syz  Szx \n");
...
//	  iselect=1;
	  ...
	  frdheader(&icounter,&oner,time,&pi,noddiam,cs,&null,mode,
		    &noutloc,description,kode,nmethod,fptr,output,istep,iinc);

//	  fprintf(fptr," -4  SIGMA      6    1\n");
//	  fprintf(fptr," -5  SXX         1    4    1    1\n");
//	  fprintf(fptr," -5  SYY         1    4    2    2\n");
//	  fprintf(fptr," -5  SZZ         1    4    3    3\n");
//	  fprintf(fptr," -5  SXY         1    4    1    2\n");
//	  fprintf(fptr," -5  SYZ         1    4    2    3\n");
//	  fprintf(fptr," -5  SZX         1    4    3    1\n");
