...
double pstiff=0;    //Penalty p for density variable from topology optimization
double *rhoPhys = NULL;   // Pointer to store density variables from topology optimization    
...
//printf("************************************************************\n\n");
//printf("You are using an executable made on Thu Jun 24 02:49:31 CDT 2021\n");
printf("\n************************************************************\n\n");
printf("MDO STRUCTURAL MODULE : Author Prateek Ranjan \n");
printf("Modified to read rho variables: Ghanendra K Das,23 June,2021");
//printf("CalculiX comes with ABSOLUTELY NO WARRANTY. This is free\n");
//printf("software, and you are welcome to redistribute it under\n");
//printf("certain conditions, see gpl.htm\n\n");
printf("************************************************************\n\n");
...
//    nobject=nobject_;
    ...
    NNEW(rhoPhys,double,ne_); // Density variables from topology optimization, allocate ne elements
    /* property cards */
  ...
  FORTRAN(calinput,(co,&nk,kon,ipkon,lakon,&nkon,&ne,
    ...
    rho(rhoPhys,ne,pstiff); //Obtain density filter from file
  ...
  if((nmethod<=1)||(nmethod==11)||((iperturb[0]>1)&&(nmethod<8)))
	...
	for(i=0;i<3;i++){nzsprevstep[i]=nzs[i];}
	  ...
	  arpackcs(co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,
	     ...
	     &pslavsurf,&clearini,&nmat,typeboun,ielprop,prop,orname, rhoPhys,&pstiff);
...
SFREE(rhoPhys); //free density rho vectore memory 
