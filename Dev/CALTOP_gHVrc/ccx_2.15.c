...
int main(int argc, char *argv[])
{
  ...
  double *designFiltered = NULL, *designFilteredH = NULL, *gradComplFiltered = NULL, *gradComplFiltered2 = NULL, *eleVolFiltered = NULL, *eleVolFiltered2 = NULL;
  ...
  double eta_H = 0.5, beta_H = 0.0000001;
  double *dHSVector = NULL;
  ...
  if (argc == 1)
  {
    printf("Usage: Flags: -i jobname -p PENALTY -q HIGHERORDERFILTER -r RADIUS -v VOLUMEFRACTION -s ITERATION -f FILTERNNZ -e eta -b beta \n");
    FORTRAN(stop, ());
  }
  else
  {
    ...
    for (i = 1; i < argc; i++)
    {
      if (strcmp1(argv[i], "-b") == 0)
      {
        beta_H = atof(argv[i + 1]);
        break;
      }
    }
   ...
   */
    ...
    if ((nmethod <= 1) || (nmethod == 11) || ((iperturb[0] > 1) && (nmethod < 8)))
    {
      if (iperturb[0] < 2)
      {
        ...
        printf("Beta for Heaviside projection= %f \n", beta_H);
        printf("\n ***********************\n\n");
        fflush(stdout);
        ...
        if (pSupplied != 0)
        {
          ...
          NNEW(designFiltered, double, ne_);  // To store density filtered rho
          NNEW(designFilteredH, double, ne_); // To store heaviside fiiltered rho
          NNEW(dHSVector, double, ne_);       // To store dheavisiderho_drho
          ...
          // Multiply a vector with H matrix from density filter
          ...
          /**
           * simple bisection search for volume preserving eta
           * To do: convert to multithreaded
           **/

          calculate_eta(designFiltered, ne, &beta_H, &eta_H);

          // Apply heaviside function to filtered density, also calculates derivative
          heavisidefilter(&ipkon, &ne, designFiltered, designFilteredH, dHSVector, &beta_H, &eta_H);

          rhoPhys = designFilteredH;
        }
        else
        {
          rhoPhys = design;
    ...
    else if (pSupplied != 0)
    {
      ...
      NNEW(gradComplFiltered2, double, ne_); // allocate memory to gradcompliance, initialize to 0
      NNEW(eleVolFiltered, double, ne_);     // allocate memory to element volume, initialize to 0
      NNEW(eleVolFiltered2, double, ne_);    // allocate memory to element volume, initialize to 0
      ...
      dheavisidefilter(&ipkon, &ne, gradCompl, gradComplFiltered2, dHSVector);                                                                                          // gradComplFilter2 = gradCompl*dHSVEctor
      filterVector(&ipkon, gradComplFiltered2, gradComplFiltered, FilterMatrixs, filternnzElems, rowFilters, colFilters, &ne, &ttime, timepar, &fnnzassumed, &qfilter); // Filter Compliance sensitivity
      // Apply heaviside function derivative

      dheavisidefilter(&ipkon, &ne, eleVol, eleVolFiltered2, dHSVector);
      filterVector(&ipkon, eleVolFiltered2, eleVolFiltered, FilterMatrixs, filternnzElems, rowFilters, colFilters, &ne, &ttime, timepar, &fnnzassumed, &qfilter); // Filter volume sensitivity

      ends = time(NULL);
      printf("Time taken for sensitivity.c is %.4f seconds \n",
      ...
      printf("Eta for Heaviside projection= %15f \n", eta_H);
  ...
  SFREE(gradComplFiltered2);
  SFREE(eleVolFiltered2);
  SFREE(dHSVector);
