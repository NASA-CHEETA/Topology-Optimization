...
!
     ...
     &  uku,penal,rhoi,elvol,xcg,ycg,zcg, k_min,
     &  rho_c, uf
!
...
c     open (unit=200,file="debug_ec3d.dat")
C      open (unit=300,file="ff_unpenal.dat")
...
!         Evaluate u*k*u and u*df if body force exists
         do i=1,3*nope
...
!        Assemble compliance and sensitivity

      if(grav_flag.eq.1) then
            if(rhoi.gt.rho_c) then
               sensi=2*uf-penal*(rhoi**(penal-1))*uku
               ecompli = (rhoi**penal)*uku
            else
               sensi = 2*uf-rho_c**(penal-1)*uku
               ecompli = rhoi*rho_c**(penal-1)*uku
            endif
      else 
